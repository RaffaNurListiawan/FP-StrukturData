...
kartu_anggota data[100], jml_anggota,temp_anggota;
...
data_peminjaman juml[100],temp_pinjam;
...
void reg_anggota() // Untuk registrasi atau mendaftarkan anggota baru
{
    ...
    cout << "Registrasi Anggota Perpustakaan Amikom\n" << endl;
    cout << "Masukkan nama: ";
...
void cek_anggota() // Mengecek anggota perpustakaan pribadi
{
    ...
    for (i = 0; i < jml_anggota.jumlah_anggota; i++)
    {
        ...
        {
            system("cls");
            cout << "==== Kartu Anggota Perpustakaan Amikom ====\n" << endl;
...
void bubbleshort(){
    int v,w;
    for (v=0; v<jml_anggota.jumlah_anggota; v++){
        for (w=0; w<jml_anggota.jumlah_anggota-1; w++){
            if (data[w].id_anggota > data[w+1].id_anggota){
                temp_anggota = data[w];
                temp_pinjam = juml[w];
                data[w]=data[w+1];
                juml[w]=juml[w+1];
                data[w+1]=temp_anggota;
                juml[w+1]=temp_pinjam;
            }
        }
    }
}
...
void search(){
    ...
    cin >> cari;
    ketemu = 0;
    for (cl =0; cl <jml_anggota.jumlah_anggota; cl++){
        if(data[cl].nama_anggota  == cari){
            ketemu = 1;
            posisi = cl;
        }
    }
    if(ketemu == 0){
        ...
        cout << "Id : " << data[posisi].id_anggota << endl;
        cout << "Jumlah buku dipinjam : " << juml[posisi].jumlah_pinjam << endl;
    } 
}
...
void list_anggota() // Ibarat seperti menjadi administrator, nantinya akan menampilkan keseluruhan data anggota serta jumlah buku yang dipinjam
{
    ...
    if (strcmp(password, "12345") == 0 && strcmp(username, "admin") == 0) {
        ...
        {
            system("cls");
            cout << "Daftar Anggota Perpustakaan Amikom\n\n";
...
int main() {
    ...
    buku[2].nama_buku = "Laskar Pelangi";                                   buku[2].id_buku = "MD-349";
    buku[3].nama_buku = "Logika Pemrograman C++";           			    buku[3].id_buku = "SD-105";
    buku[4].nama_buku = "Kamu Terlalu Banyak Bercanda";      			    buku[4].id_buku = "AL-458";
    buku[5].nama_buku = "Tereliye";         								buku[5].id_buku = "PK-566";
    buku[6].nama_buku = "All The Bright Places"; 						    buku[6].id_buku = "PA-612";
